...
(function (window, $) {
    ...
    var other, html, i, mixLoading = null,
        meTables = function (options) {
            ...
            this.init = function (params) {
                ...
                //console.log(self.options.buttonSelector,self.options.buttonType,self.options.buttonHtml);
                // 添加按钮
                try {
            ...
            this.save = function (data) {
                ...
                //console.log($("select[name='next_admin_id']").attr("disabled"));
                // 第二步：新增和修改验证数据、数据的处理
            ...
            this.getUrl = function (strType) {
                //console.log(this.options.url[strType]);
            ...
            this.initForm = function (data) {
                ...
                $(m).modal({backdrop: "static"});   // 弹出信息 backdrop:true 点击弹出层外关闭
            };
    ...
    meTables.selectCreate = function (params, d) {
        ...
        if (d && this.isObject(d)) {
            ...
            if(params.prompt!== undefined){
                html += "<option value=''>" + params.prompt + "</option>";
            }
            ...
            if(typeof(params.sort) == "undefined"){
                ...
                if(!params.sort){//当不重排选择项数组时
                    for (i in d) {
                        html += '<option value="' + d[i]['value'] + '" ' + (i == c ? ' selected="selected" ' : "") + ">" + d[i]['title'] + "</option>";
                    }
                }
            }
    ...
    meTables.buttonsCreate = function (index, data, unique, rowArray) {
        ...
        if (data !== undefined && typeof data === "object") {
            for (var i in data) {
                ...
                div1 += ' <button title="' + data[i]['title'] + '" class="btn ' + data[i]['className'] + ' ' + data[i]['cClass'] + '-' + unique + ' btn-xs" data-row="' + index + '">';
    ...
    meTables.selectInput = function (params, value, defaultObject) {
        ...
        if (value) {
            if(typeof(params.sort) == "undefined"){
                ...
                if(!params.sort){//当不重排选择项数组时
                    for (i in value) {
                        html += '<option value="' + value[i]['value'] + '">' + value[i]['title'] + '</option>';
                    }
                }
            }
        }
    ...
    // 分类显示
    meTables.cateString = function (data, value) {
        return data[value];
    };
