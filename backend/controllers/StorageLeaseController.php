 ...
 */
class StorageLeaseController extends Controller
{
    protected $pk = 'k_id';
    ...
    {
        $access = $this->get_access(5,Yii::$app->session['g_id'],3);
    ...
    public function actionCreate()
    {
        ...
        if(!empty($post)){
            if ($model->load(Yii::$app->request->post()) && $model->save()) {
                return $this->redirect(['/storage-lease/update?id='.$model->k_id]);
            }
        }
    ...
    public function actionUpdate()
    {
        ...
        if(!empty($post)){
            ...
            if(!empty($post['DtoaStorageCart'])){
                $n=count($post['DtoaStorageCart']['jc_id']);
                for ($i = 0; $i < $n; $i++) {
                    $cartmodel = new DtoaStorageCart();
                    $cartmodel->k_id=$model->k_id;
                    $cartmodel->j_id=0;
                    $cartmodel->j_no='';
                    $cartmodel->h_id=$post['DtoaStorageCart']['h_id'][$i];
                    $cartmodel->h_no=$post['DtoaStorageCart']['h_no'][$i];
                    ...
                    $cartmodel->lease_time=empty($post['DtoaStorageCart']['lease_time'][$i])?0:$post['DtoaStorageCart']['lease_time'][$i];
                    $cartmodel->real_time=empty($post['DtoaStorageCart']['real_time'][$i])?'':$post['DtoaStorageCart']['real_time'][$i];
                    $cartmodel->lease_time_unit=$post['DtoaStorageCart']['lease_time_unit'][$i];
                    ...
                    $cartmodel->over_num=empty($post['DtoaStorageCart']['over_num'][$i])?0:1;
                    $cartmodel->over_time=empty($post['DtoaStorageCart']['over_time'][$i])?0:1;
                    $cartmodel->one_price=bcmul($cartmodel->j_real_num,$cartmodel->j_real_price);
                    if(!empty($cartmodel->real_time)){
                        $cartmodel->one_price=bcmul($cartmodel->one_price,$cartmodel->real_time);
                    }
                    ...
                    if(!$cartmodel->save()){
                        ...
                        if($cartmodel->storage_ok==1){
                            ...
                            if($oacartmodel->lease_time==0){
                                $surplus_time=0;
                            }else{
                                $surplus_time=bcsub($oacartmodel->surplus_time,$cartmodel->real_time);
                                if($surplus_time<=0){
                                    $surplus_time=0;
                                }
                            }
                        ...
                        $oacartmodel->surplus_time=$surplus_time;
                        if(!$oacartmodel->save()){
        ...
        $storagecart=DtoaStorageCart::find()->select(['dtoa_storage_cart.*','m.c_no','m.c_name','m.size','m.quality','m.num'])->andWhere(['k_id'=>$model->k_id])
