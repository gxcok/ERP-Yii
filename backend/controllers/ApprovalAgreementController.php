...
class ApprovalAgreementController extends Controller
{
    ...
    {
        $access = $this->get_access(4,Yii::$app->session['g_id']);
    ...
    public function actionUpdate()
    {
        ...
        if(Yii::$app->request->post()){
            ...
            $uid_arr=DtoaApprovalGroup::find()->select(['uid_arr'])->andWhere(['g_id'=>$model->g_id])->andWhere(['item_id'=>4])->asArray()->one();
            ...
            }else{//拒绝则 agreement h_status=0 next_admin_id=第一个审批人
                /*$a_id_arr=DtoaApproval::find()->select(['a_id'])->andWhere(['g_id' => $model['g_id']])->andWhere(['table_index' => 2])->andWhere(['target_id' => $model['h_id']])->asArray()->all();
            ...
            $Approvalmodel->table_index=2;
            $Approvalmodel->target_id=$model->h_id;
