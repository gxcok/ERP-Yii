...
class PlanController extends Controller
{
    ...
    {
        $access = $this->get_access(1,Yii::$app->session['g_id']);
    ...
    public function actionCreate()
    {
        ...
        if(!empty($post)){
            if ($model->load(Yii::$app->request->post()) && $model->save()) {
                return $this->redirect(['/plan/update?id='.$model->j_id]);
            }
        }
        ...
        if(!empty($post)){
            ...
            if(!empty($post['DtoaCart'])){
                ...
                for ($i = 0; $i < $n; $i++) {
                    ...
                    $cartmodel->surplus_num=$post['DtoaCart']['j_num'][$i];
            ...
            $model->j_name=$post['DtoaPlan']['j_name'];
            $model->j_status=$post['DtoaPlan']['j_status'];
            $model->need_date=$post['DtoaPlan']['need_date'];
            $model->j_info=$post['DtoaPlan']['j_info'];
            $total_price=DtoaCart::find()->select(['total_price'=>'SUM(one_price)'])->andWhere(['j_id'=>$model->j_id])->asArray()->one()['total_price'];
