 ...
 */
class ApprovalPrePlanController extends Controller
{
    //public $layout=false;
    ...
    {
        $access = $this->get_access(12,Yii::$app->session['g_id']);
    ...
    public function actionUpdate()
    {
        ...
        if(Yii::$app->request->post()){
            ...
            $uid_arr=DtoaApprovalGroup::find()->select(['uid_arr'])->andWhere(['g_id'=>$model->g_id])->andWhere(['item_id'=>12])->asArray()->one();
            ...
            }else{//拒绝则 plan j_status=1 next_admin_id=第一个审批人 删除所有审批记录
        ...
        $plancart=DtoaPreCart::find()->select(['dtoa_precart.*','m.c_no','m.c_name','m.size','m.quality','m.num'])->andWhere(['j_id'=>$model->j_id])
            ->join('INNER JOIN', ['m' => 'dtoa_material'], 'm.c_id = dtoa_precart.c_id')->asArray()->all();
