...
class PrePlanController extends Controller
{
     ...
     */
    public $modelClass = 'backend\models\DtoaPrePlan';
    ...
    {
        $access = $this->get_access(11,Yii::$app->session['g_id']);
    ...
    public function actionCreate()
    {
        ...
        if(!empty($post)){
            if ($model->load(Yii::$app->request->post()) && $model->save()) {
                return $this->redirect(['/pre-plan/update?id='.$model->j_id]);
            }else {
    ...
    public function actionUpdate()
    {
        ...
        if(!empty($post)){
            ...
            if(!empty($post['DtoaCart'])){
                ...
                for ($i = 0; $i < $n; $i++) {
                    $cartmodel = new DtoaPreCart();
                    $cartmodel->j_id=$model->j_id;
                    $cartmodel->c_id=$post['DtoaCart']['c_id'][$i];
                    $cartmodel->j_num=$post['DtoaCart']['j_num'][$i];
                    ...
                    $add_price=bcadd($add_price,bcmul($cartmodel->j_num,$cartmodel->j_price));
                }
            }
            $model->s_id=$post['DtoaPrePlan']['s_id'];
            $model->j_name=$post['DtoaPrePlan']['j_name'];
            $model->j_status=$post['DtoaPrePlan']['j_status'];
            $model->need_date=$post['DtoaPrePlan']['need_date'];
            $model->j_info=$post['DtoaPrePlan']['j_info'];
            $model->total_price=bcadd($model->total_price,$add_price);
            if(!$model->save()){
                //var_dump($model->getErrors());
            }else{
        ...
        $plancart=DtoaPreCart::find()->select(['dtoa_precart.*','m.c_no','m.c_name','m.size','m.quality','m.num'])->andWhere(['j_id'=>$model->j_id])
            ->join('INNER JOIN', ['m' => 'dtoa_material'], 'm.c_id = dtoa_precart.c_id')->orderBy('jc_id')->asArray()
            ->all();
        $approval=DtoaApproval::find()->andWhere(['g_id' => $model['g_id']])->andWhere(['table_index' => 5])
            ->andWhere(['target_id' => $model['j_id']])->asArray()->all();
    ...
    public function actionView()
    {
        ...
        $plancart=DtoaPreCart::find()->select(['dtoa_precart.*','m.c_no','m.c_name','m.size','m.quality','m.num'])->andWhere(['j_id'=>$model->j_id])
            ->join('INNER JOIN', ['m' => 'dtoa_material'], 'm.c_id = dtoa_precart.c_id')->orderBy('jc_id')->asArray()->all();
        $approval=DtoaApproval::find()->andWhere(['g_id' => $model['g_id']])->andWhere(['table_index' => 5])
            ->andWhere(['target_id' => $model['j_id']])->asArray()->all();
    ...
    public function getExportHandleParams()
    {
        ...
        $array['j_status'] = function ($value) {
            return DtoaPrePlan::PLAN_STATUS[$value];
        };
    ...
    {
        if (($model = DtoaPrePlan::findOne($id)) !== null) {
            return $model;
        }
