...
<div class="dtoa-plan-update">
    <div class="dtoa-plan-form" style="">
        ...
        <div class="row">
            <div class="col-md-4">
                <?= $form->field($model, 'j_status')->dropdownList(array_slice(DtoaPlan::PLAN_STATUS,0,2))?>
            </div>
        ...
        <ul style="list-style: none; padding: 0 10px; border: 1px silver dashed; margin: 0 0 10px;">
            <h2 style="margin: 5px 0;">
                <a class="btn btn-primary btn-sm" id="add_plancart" data-j-id="<?=$model['j_id']?>">选择计划材料</a>

                <a class="btn btn-primary btn-sm" id="show_preplan" data-s-id="<?=$model['s_id']?>">查看采购意向单</a>
            </h2>
...
<script>
    $(function(){
        ...
        $("#show_preplan").bind('click',function(){
            layer.open({
                type: 2,
                //offset: '50px',
                title: '采购意向单',
                shade: 0.8,
                area: ['1200px', '600px'],
                content: '/pre-plan/select?plan_s_id='+$(this).attr('data-s-id'),
            });
        });
        ...
        $(".cart-save").bind('click',function(){
            ...
            if(parseFloat(j_num)>0 && parseFloat(j_price)>0){
                $.ajax({
                    type: "POST",
                    url: "/cart/update",
                ...
                layer.msg('数量和单价不能为空 和 <=0', {icon: 6});
            }
        });
        ...
        $(".cart-del").bind('click',function(){
            ...
            layer.confirm('确认删除计划材料?', {icon: 3, title: '删除计划材料'}, function (index) {
                ...
                layer.close(index);
                $("#dtoaplan-total_price").val(sum_total_price());
            });
        });
    });
    ...
    $(".update-save").bind('click',function(){
        ...
        if(check){
            if($('#dtoaplan-j_status').val()==1 && $("#plancart-box tr").length<=0){
