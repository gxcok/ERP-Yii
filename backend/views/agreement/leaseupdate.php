            ...
            <table class="table table-bordered table-condensed table-striped" style="margin-top: 10px;border: 1px solid #ddd;">
                ...
                <?php foreach ($agreementcart as $k => $cart) { ?>
                    <tr data-jc_id="<?=$cart['jc_id']?>" data-h_id="<?=$model['h_id']?>" data-c_id="<?=$cart['c_id']?>">
                        ...
                        <td><?=$cart['quality']?></td>
                        <td><?=$cart['num']?></td>
                        <td><input type="text" autocomplete="off" class="edit_input j_num_item" value="<?=empty(floatval($cart['j_num']))?'':floatval($cart['j_num'])?>"></td>
                        <td><input type="text" autocomplete="off" class="edit_input j_price_item" value="<?=floatval($cart['j_price'])?>"></td>
                        <td><input type="text" autocomplete="off" class="edit_input lease_time_item" value="<?=empty(floatval($cart['lease_time']))?'':floatval($cart['lease_time'])?>"></td>
                        <td><input type="text" class="edit_input lease_time_unit_item" value="<?=$cart['lease_time_unit']?>"></td>
...
<script>
    ...
    $(function(){
        ...
        $(".cart-save").bind('click',function(){
            ...
            var lease_time = $(this).parent('td').parent('tr').find('.lease_time_item').val();
            var lease_time_unit = $(this).parent('td').parent('tr').find('.lease_time_unit_item').val();
            var jc_info = $(this).parent('td').parent('tr').find('.jc_info_item').val();
            if(parseFloat(j_price)>0){
                $.ajax({
                    type: "POST",
                    url: "/cart/update",
                    data: {'jc_id':jc_id,'h_id':h_id,'j_num':j_num,'j_price':j_price,'lease_time':lease_time,'lease_time_unit':lease_time_unit,'jc_info':jc_info},
                    ...
                    if(data.error){
                        ...
                        for (i = 0; i < data.length; i++) {
                            input_html=input_html+"<tr class='m-add' data-index='"+(parseInt(i)+n+1)+"' data-c_id='"+data[i]['c_id']+"'>" +
                                ...
                                "<td>"+data[i]['num']+"</td>" +
                                "<td><input type='text' autocomplete=\"off\" name='DtoaCart[j_num][]' class='j_num_input'></td>" +
                                "<td><input type='text' autocomplete=\"off\" name='DtoaCart[j_price][]' class='j_price_input'></td>" +
                                "<td><input type='text' autocomplete=\"off\" name='DtoaCart[lease_time][]' class='lease_time_input'></td>" +
                                "<td><input type='text' name='DtoaCart[lease_time_unit][]' class='lease_time_unit_input'></td>" +
                        ...
                        $('input.lease_time_input').each(function(){
                            $(this).bind('keyup',function() {
                                $(this).val($(this).val().replace(/^(\d+)\.(\d{0,3}).*$/,'$1.$2'));
                                var lease_time = $(this).val();
                        ...
                        $('input.j_price_input').each(function(){
                            $(this).bind('keyup',function() {
                                $(this).val($(this).val().replace(/^(\d+)\.(\d{0,3}).*$/,'$1.$2'));
    ...
    $(".update-save").bind('click',function(){
        ...
        $("tr.m-add").each(function(){
            ...
            var lease_time = $(this).find(".lease_time_input").val();
            var lease_time_unit = $(this).find(".lease_time_unit_input").val();
            if(j_price=='' || parseFloat(j_price)<=0){
                layer.msg('第'+n+'行 [租赁单价] 不能为空 和 <=0', {icon: 6});
                check = false;
            }
        });
        if(check){
    ...
    $('input.j_num_item').each(function(){
        $(this).bind('keyup',function() {
            $(this).val($(this).val().replace(/^(\d+)\.(\d{0,3}).*$/,'$1.$2'));
            var j_num = $(this).val();
            var j_price = $(this).parent('td').parent('tr').find('.j_price_item').val();
    ...
    $('input.lease_time_item').each(function(){
        $(this).bind('keyup',function() {
            $(this).val($(this).val().replace(/^(\d+)\.(\d{0,3}).*$/,'$1.$2'));
            var lease_time = $(this).val();
            var j_num = $(this).parent('td').parent('tr').find('.j_num_item').val();
    ...
    $('input.j_price_item').each(function(){
        $(this).bind('keyup',function() {
            $(this).val($(this).val().replace(/^(\d+)\.(\d{0,3}).*$/,'$1.$2'));
            var j_price = $(this).val();
            var j_num = $(this).parent('td').parent('tr').find('.j_num_item').val();
