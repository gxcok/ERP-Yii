...
<head>
    ...
    <style>
        ...
        #material_box,#material_select_box{height:210px;min-width:600px;overflow:scroll;padding:0;}
...
<script type="text/javascript">
        ...
        $('.cname').bind('click', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    //console.log(jsondata);//m_table label_arr
                    var m_html = '';
                    for (var i = 0; i < data.length; i++) {
                        m_html = m_html + '<tr data-c_id="'+data[i]['c_id']+'"><td><input type="checkbox" ' +
                            'name="select-m" class="select-m" value="'+data[i]['c_id']+'"></td>';
                        for (k in label_arr) {
    ...
    $('#select_ok').bind('click',function() {
        ...
        var vals = arr.join(",");
        if(vals){
            if($("ul.cname.ul-selected").length>0){
                var select_cid=$("ul.cname.ul-selected").attr('data-id');
            }else{
                var select_cid=0;
            }
            parent.create_input(vals,'material-select',select_cid);
            //当你在iframe页面关闭自身时
    ...
    $('#move_out').bind('click',function() {
        $("#material_select_box .select-m:checked").each(function(i){
