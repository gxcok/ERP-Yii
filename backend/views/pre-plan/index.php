...
$plan=DtoaPrePlan::find()->select(['j_id'])->andWhere(['g_id'=>Yii::$app->session['g_id']])->andWhere(['>=','j_status',2])->asArray()->all();
?>
<?=MeTable::widget()?>
    <style>
...
<script type="text/javascript">
    ...
    var jStatus = <?=Json::encode(DtoaPrePlan::PLAN_STATUS)?>;

    var jStatusColor = <?=Json::encode(DtoaPrePlan::PLAN_STATUS_COLOR)?>;
    ...
    $(document).on("click", "#show-table-add", function () {
        parent.addIframe(GetRandomNum(1000,100000),'/pre-plan/create','添加材料意向单');
    });
    ...
    $(document).on("click", ".me-table-more-show-table", function () {
        ...
        // 自定义处理逻辑
        //console.log(data);
        parent.addIframe(GetRandomNum(1000,100000),'/pre-plan/update?id='+data['j_id'],'更新材料意向单');
    });
    $(document).on("click", ".me-table-view-show-table", function () {
        ...
        var data = m.table.data()[row];
        // 自定义处理逻辑
        parent.addIframe(GetRandomNum(1000,100000),'/pre-plan/view?id='+data['j_id'],'查看材料意向单');
    });
</script>
<?php $this->endBlock(); ?>