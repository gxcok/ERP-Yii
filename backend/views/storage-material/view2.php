            ...
            solid #ddd;">
                ...
                <?php foreach ($storagecart as $k => $cart) { ?>
                    <tr data-sc_id="<?=$cart['sc_id']?>">
                        ...
                        <td><?=$cart['j_no']?></td>
                        <td><?=$cart['h_no']?></td>
                        <td><div style="width:180px;height:30px;"><input type="text" class="edit_input j_num_item
                        num_check" value="<?=floatval($cart['j_real_num'])?>" readonly>
                            <span style="float: left;width:80px;">
                            <input type='hidden' class="surplus_num" value="<?=$cart['surplus_num']?>">
                        ...
                        <td><?php
                            if($cart['j_num']==0){
                                ...
                                echo floatval($cart['surplus_num']);
                            }
                            ?></td>
...
<div id="page-foot" style="display: none">
    <ul style="width: 100%;display:block;height:22px; line-height:22px;list-style:none;margin: 0;padding:0 20px;page-break-after: always;">
...
if($.inArray(head[i].k_name,['total_price','j_num','j_price','mul_price', 'j_real_num','j_real_price','pour_num'])!=-1){
                            ...
                            if(i%4==0){
                                model_th='<thead><tr><th>'+head[i].k_title+'：</th>';
                                ...
                                model_th=model_th+'<th>'+head[i].k_title+'：</th></tr></thead>';
                                if(print_data.model[head[i].k_name]==null){
                                ...
                                model_th=model_th+'<th>'+head[i].k_title+'：</th>';
                                if(print_data.model[head[i].k_name]==null){
                        ...
                        var page_height=800;
                        var tr_height=20;
                        var model_height=220;   //$("#print_model").height()
                        var total_page=Math.ceil(((cart.length * tr_height)+model_height)/(800-30));
                        console.log(total_page);
                        var cart_head_html='<thead><tr>';
                        ...
                        for (p = 1; p < total_page; p++) {
                            var cart_table_html = '<table class="table table-bordered table-condensed" style="font-size:12px;">';
                            cart_table_html=cart_table_html+cart_head_html;
                            var cart_html='<tbody>';
                            if(p==1){
                        ...
                        for (i = 0; i < cart.length; i++) {
                            ...
                            cart_table_html=cart_table_html+cart_html+'</table>';
                            $("#print_cart").append(cart_table_html);
                            $("#print_cart").append($("#page-foot").html());
                        }
                        $("#print_box").show();
                        //printdiv('print_box');
                        /*$("#print_box").printArea({
