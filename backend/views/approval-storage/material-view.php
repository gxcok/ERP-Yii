...
<div class="dtoa-storage-update">
    <div class="dtoa-storage-form" style="">
        ...
        <div class="row">
            <div class="col-md-4">
                <?=$form->field($model, 'next_admin_id')->dropdownList($user,['disabled' => 'true'])?>
            </div>
            <div class="col-md-4">
            ...
            solid #ddd;">
                ...
                <?php foreach ($storagecart as $k => $cart) { ?>
                    <tr data-sc_id="<?=$cart['sc_id']?>">
                        ...
                        <td><?=$cart['j_no']?></td>
                        <td><?=$cart['h_no']?></td>
                        <td><input type="text" class="edit_input j_num_item num_check" value="<?=floatval($cart['j_real_num'])?>" readonly>
...
<script>
    ...
    $(function(){
        $('input.num_check').each(function(){
            if(k_source=='计划入库'){
                if(parseFloat($(this).val())>=parseFloat($(this).next('.surplus_num').val())){
                    $(this).next(".surplus_num").next('.j_num').next(".storage_ok").prop("checked",true);
                }else{
                    $(this).next(".surplus_num").next('.j_num').next(".storage_ok").prop("checked",false);
                }
                if($(this).next(".surplus_num").next('.j_num').next(".storage_ok").val()==1){
                    $(this).next(".surplus_num").next('.j_num').next(".storage_ok").prop("checked",true);
                }
                $(this).next(".surplus_num").next('.j_num').next(".storage_ok").attr("disabled",true);
                if(parseFloat($(this).val())>parseFloat($(this).next('.surplus_num').val())){  //数量超出标红
