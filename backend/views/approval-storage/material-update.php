        ...
        $('input.num_check').each(function(){
            if(k_source=='计划入库'){
                if(parseFloat($(this).val())>=parseFloat($(this).next('.surplus_num').val())){
                    $(this).next('.j_num').next(".storage_ok").prop("checked",true);
                }else{
                    $(this).next('.j_num').next(".storage_ok").prop("checked",false);
                }
                if($(this).next('.j_num').next(".storage_ok").val()==1){
                    $(this).next('.j_num').next(".storage_ok").prop("checked",true);
                }
                $(this).next('.j_num').next(".storage_ok").attr("disabled",true);
