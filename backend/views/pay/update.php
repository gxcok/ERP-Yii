...
$supplier[$model->s_id]=DtoaSupplier::findOne($model->s_id)['s_name'];
    ...
    <div class="dtoa-pay-form">
        ...
        <div class="row">
            <div class="col-md-4"><?php
                ...
                echo $form->field($model, 's_id')->dropdownList($supplier);
?></div>
        ...
        <ul id="storagelist" style="list-style: none; padding: 0 10px; border: 1px silver dashed; margin: 10px 0;">
            ...
            solid #ddd;font-size: 13px;">
                ...
                <tbody>
                <?php
                if(!is_array($model->p_data)){
                    $k_id_arr=json_decode($model->p_data);
                }else{
                    $k_id_arr=$model->p_data;
                }
                if(empty($k_id_arr)){
                    $k_id_arr=[];
                }
                foreach ($storagelist as $k => $s) { ?>
                    <tr>
                        <td><input type="checkbox" class="checkbox storage" data-id="<?=$s['k_id']?>"
                                   data-price="<?=floatval($s['total_price'])?>" <?php if(in_array($s['k_id'],$k_id_arr)){?>checked<?php
                                   }?>></td>
                        <td><?=$s['k_id']?></td>
                        <td><?=$s['k_no']?></td>
...
<script>
    var p_data = <?=Json::encode($model['p_data'])?>;
    $('#dtoapay-p_data').val(p_data);
    $(function(){
        $('#dtoapay-s_id').bind('change',function() {
            console.log($('#dtoapay-p_data').val());
            if($('#dtoapay-p_data').val()=='[]'||$('#dtoapay-p_data').val()==''){
                location.href="/pay/update?id=<?=$model['p_id']?>&s_id="+$(this).val();
            }else{
                layer.msg('请取消当前供应商已选择的入库单，保存再切换供应商！', {icon: 6});
                $('#dtoapay-s_id').val(<?=$model['s_id']?>);
            }
        });
    });
    ...
    $('#list_all').bind('click',function() {
        ...
        if($(this).is(':checked')){
            ...
            $('#dtoapay-total_price').val(parseFloat(sum));
        }else{
    ...
    $('.save-btn').bind('click',function() {
        if($('#dtoapay-p_status').val()==1 && $('#dtoapay-p_data').val()=='[]'){
