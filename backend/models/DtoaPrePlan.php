...
/**
 * This is the model class for table "dtoa_preplan".
 *
 ...
 */
class DtoaPrePlan extends \yii\db\ActiveRecord
{
    const PLAN_STATUS=[
        0 => '正在筹备',
        1 => '等待审批',
    ...
    {
        return [
            [['g_id', 'j_no', 'j_name', 'next_admin_id','creator', 'created_at', 'creator_id'], 'required'],
    ...
    {
        $no=DtoaPrePlan::find()->select(['j_no'])->andWhere(['g_id'=>Yii::$app->session['g_id']])->orderBy('j_id desc')->asArray()->one();
        if(empty($no)){
            return '001';
        }else{
